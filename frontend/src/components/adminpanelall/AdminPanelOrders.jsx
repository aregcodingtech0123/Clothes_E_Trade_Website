...
const AdminOrders = () => {
  ...
  ? orders.filter(order =>
      ...
      String(order.customer?.name || '').toLowerCase().includes(searchTerm.toLowerCase())
    )
  : [];
      ...
      <div className="container mx-auto px-4 py-8 min-h-screen">
        ...
        <div className="mb-6 relative">
          ...
          <input
            ...
            className="pl-10 pr-4 py-2 border rounded-lg w-full max-w-md focus:outline-none focus:ring-2 focus:ring-amber-500"
            value={searchTerm}
          ...
          <div className="bg-white rounded-lg shadow overflow-hidden">
            {filteredOrders.length === 0 ? (
              <div className="p-6 text-center text-gray-500">
                {searchTerm ? 'Aramanızla eşleşen sipariş bulunamadı' : 'Henüz sipariş bulunmamaktadır'}
              </div>
            ) : (
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {filteredOrders.map((order) => (
                    <tr key={order.id}>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{order.user?.name || 'Bilinmiyor'}</td>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                        {(order.totalAmount ? Number(order.totalAmount).toFixed(2) : 
                          (Array.isArray(order.orderItems) 
                            ? order.orderItems.reduce((sum, item) => sum + (Number(item.price) * (item.quantity || 0)), 0).toFixed(2) 
                            : '0.00'))} TL
                      </td>
