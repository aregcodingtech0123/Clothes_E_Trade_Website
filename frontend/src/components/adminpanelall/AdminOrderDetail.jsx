    ...
    const fetchOrder = async () => {
      try {
        ...
        if (parsedOrder.totalAmount) {
          ...
          setShippingFee(20.00);
          setSubtotal(totalFromAPI - 20.00);
        } else {
        ...
        <div className="flex justify-between items-center mb-6">
          ...
          <div className="flex items-center space-x-4">
            ...
            <select
              ...
              disabled={isUpdating}
              className="border rounded px-3 py-1 focus:outline-none focus:ring-2 focus:ring-amber-500 disabled:opacity-50"
            >
        ...
        <div className="bg-white rounded-lg shadow overflow-hidden mb-6">
          <div className="p-6 border-b">
            ...
            <p className="text-gray-500">
              Sipariş Tarihi: {order.createdAt ? new Date(order.createdAt).toLocaleDateString('tr-TR', {
                year: 'numeric',
                month: 'long',
                day: 'numeric',
                hour: '2-digit',
          ...
          <div className="p-6">
            ...
            {!order.orderItems || order.orderItems.length === 0 ? (
              <div className="text-center py-8">
                <p className="text-gray-500 mb-2">Bu siparişte ürün detayları bulunamadı.</p>
                <p className="text-sm text-gray-400">
                  Toplam Tutar: {order.totalAmount ? parseFloat(order.totalAmount).toFixed(2) : '0.00'} TL
                </p>
              </div>
            ) : (
              <div className="divide-y divide-gray-200">
                {order.orderItems.map((item, index) => {
                  ...
                  const quantity = parseInt(item.quantity) || 0;
                  return (
                    <div key={item.id || index} className="py-4 flex justify-between">
                      <div>
                      ...
                      <span className="font-semibold">{(price * quantity).toFixed(2)} TL</span>
                    </div>
                  );
                })}
              </div>
        ...
        <div className="grid md:grid-cols-2 gap-6">
          <div className="bg-white rounded-lg shadow p-6">
            <h2 className="text-xl font-semibold flex items-center mb-4">
              <User className="mr-2 text-amber-500" />
              Müşteri Bilgileri
            </h2>
            <div className="space-y-2">
              <p><span className="font-medium">Ad:</span> {order.user?.name || 'Belirtilmemiş'}</p>
              <p><span className="font-medium">Email:</span> {order.user?.email || 'Belirtilmemiş'}</p>
              <p><span className="font-medium">Müşteri ID:</span> {order.user?.id || order.userId || 'Belirtilmemiş'}</p>
            </div>
          </div>
          ...
          <div className="bg-white rounded-lg shadow p-6">
            ...
            <address className="not-italic space-y-2">
              <p>{shippingAddress.street || shippingAddress.address || 'Adres belirtilmemiş'}</p>
              <p>
                {shippingAddress.district || 'İlçe belirtilmemiş'} / {shippingAddress.city || 'Şehir belirtilmemiş'}
              </p>
              <p>{shippingAddress.country || 'Ülke belirtilmemiş'}</p>
              <p>Posta Kodu: {shippingAddress.postalCode || 'Belirtilmemiş'}</p>
              {shippingAddress.fullName && (
                <p><span className="font-medium">Alıcı:</span> {shippingAddress.fullName}</p>
              )}
          ...
          <div className="bg-white rounded-lg shadow p-6">
            ...
            <div className="space-y-2">
              <p><span className="font-medium">Yöntem:</span> {
              ...
              <p><span className="font-medium">Toplam:</span> <span className="text-amber-600 font-bold">{total.toFixed(2)} TL</span></p>
            </div>
          </div>
          ...
          <div className="bg-white rounded-lg shadow p-6">
            ...
            <button 
              ...
              disabled={isUpdating}
              className="mt-3 bg-amber-500 hover:bg-amber-600 text-white px-4 py-2 rounded-md transition-colors disabled:opacity-50"
            >
