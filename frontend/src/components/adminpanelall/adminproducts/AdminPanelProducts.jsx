  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (errorText.includes('Cannot delete product with active orders')) {
        alert('Bu ürün aktif siparişlerde bulunduğu için devre dışı bırakılamadı.');
      } else {
        ...
        <div className="flex justify-between items-center mb-8">
          ...
          {!showAddForm && !editingProduct && !deletingProduct && (
            <button
              ...
              className="flex items-center bg-amber-500 hover:bg-amber-600 text-white px-4 py-2 rounded"
            >
                      ...
                      {products.map((product) => (
                        <tr key={product.id}>
                          ...
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                            {product.category?.name || product.category || 'Belirtilmemiş'}
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
