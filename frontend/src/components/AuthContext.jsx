...
export const AuthProvider = ({ children }) => {
  ...
  const login = async (email, password) => {
    try {
      const response = await fetch('http://localhost:5000/api/auth/login', { // <-- BURAYI KENDİ BACKEND LOGIN URL'NİZLE DEĞİŞTİRİN
        method: 'POST',
        headers: {
  ...
  const signup = async (email, password, name) => {
    try {
      const response = await fetch('http://localhost:5000/api/auth/register', { // <-- BURAYI KENDİ BACKEND SIGNUP URL'NİZLE DEĞİŞTİRİN
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const addToCart = (product, quantity = 1) => {
    setCartItems(prev => {
      ...
      const variantKey = product.variant ? JSON.stringify(product.variant) : 'no-variant';
      const itemKey = `${product.id}-${variantKey}`;
      
      const existingItem = prev.find(item => {
        const itemVariantKey = item.variant ? JSON.stringify(item.variant) : 'no-variant';
        return `${item.id}-${itemVariantKey}` === itemKey;
      });
      ...
      if (existingItem) {
        updatedCart = prev.map(item => {
          const itemVariantKey = item.variant ? JSON.stringify(item.variant) : 'no-variant';
          const currentItemKey = `${item.id}-${itemVariantKey}`;
      ...
      // Show notification (use toastId to prevent duplicates)
      toast.success(t('productAddedToCart'), {
        ...
        pauseOnHover: true,
        draggable: true,
        toastId: `cart-${product.id}`, // Prevent duplicate notifications for same product
      });
