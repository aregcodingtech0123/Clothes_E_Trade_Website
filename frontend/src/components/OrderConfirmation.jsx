...
//           <div className="mt-4 inline-flex items-center px-4 py-2 rounded-full text-sm font-medium bg-amber-100 text-amber-800">
...
//               <Truck className="mr-2 text-amber-500" />
//               Sipariş Detayları
//             </h2>
//           </div>
//           <div className="p-6">
...
//                 <span>Toplam</span>
//                 <span className="text-amber-600">{lastOrder.totalAmount.toFixed(2)} TL</span>
//               </div>
//             </div>
//           </div>
//         </div>
...
//           <div className="bg-white rounded-lg shadow p-6">
//             <h2 className="text-xl font-semibold flex items-center mb-4">
//               <CreditCard className="mr-2 text-amber-500" />
//               Ödeme Bilgileri
//             </h2>
//             <div>
...
//                   lastOrder.paymentStatus === 'failed' ? 'text-red-500' : 'text-amber-500'
//                 }`}>
...
//               <div className="flex justify-between font-semibold text-lg pt-2 border-t">
//                 <span>Toplam</span>
//                 <span className="text-amber-600">{order.totalPrice.toFixed(2)} TL</span>
//               </div>
//             </div>
//           </div>
//         </div>
...
const OrderConfirmation = () => {
  ...
  useEffect(() => {
    const fetchOrder = async () => {
      try {
        ...
        // Scroll to top after order is loaded
    ...
    if (!order && token) {
      ...
      // Scroll to top if order is already available
      window.scrollTo({ top: 0, behavior: 'smooth' });
    }
  ...
  const getPaymentStatusText = (status) => {
    switch (status) {
      ...
      case 'completed':
        return t('paymentCompleted') || 'Payment Completed';
      case 'failed':
  ...
  const subtotal = order?.orderItems?.reduce((sum, item) => {
    if (!item) return sum;
    const price = typeof item.price === 'object' && item.price.toString ? Number(item.price.toString()) : Number(item.price);
  ...
  if (loading) {
    return (
      <>
        ...
        <div className="container mx-auto px-4 py-8 min-h-screen flex items-center justify-center">
          <div className="text-center">
            <p className="text-lg">{t('loadingOrders')}</p>
          </div>
        </div>
        <Footer />
      </>
  ...
  if (error || !order) {
    return (
      <>
        ...
        <div className="container mx-auto px-4 py-8 min-h-screen flex items-center justify-center">
          <div className="text-center">
            <p className="text-lg text-red-500">{error || t('orderNotFound')}</p>
            <button
              onClick={() => navigate('/')}
              className="mt-4 bg-amber-500 hover:bg-amber-600 text-white px-6 py-2 rounded-md transition-colors"
            >
  ...
  return (
    <>
      ...
      <div className="container mx-auto px-4 py-8 min-h-screen">
        <div className="max-w-3xl mx-auto text-center mb-8">
          <CheckCircle size={64} className="mx-auto text-green-500 mb-4" />
          <h1 className="text-3xl font-bold mb-2">{t('orderReceived')}</h1>
          <p className="text-gray-600">
            {t('orderNumber')}: <span className="font-semibold">{order.orderNumber}</span>
          </p>
          <p className="text-gray-600 mt-2">{t('orderDate')}: {formatDate(order.createdAt)}</p>
          <div className="mt-4 inline-flex items-center px-4 py-2 rounded-full text-sm font-medium bg-amber-100 text-amber-800">
            {getOrderStatusText(order.status)}
          </div>
        </div>
        ...
        <div className="bg-white rounded-lg shadow overflow-hidden mb-8">
          ...
          <div className="p-6 bg-gray-50">
            <div className="space-y-2">
              ...
              <div className="flex justify-between font-semibold text-lg pt-2 border-t">
                <span>{t('total')}</span>
                <span className="text-amber-600">{formatPrice(subtotal + (subtotal > 500 ? 0 : 29.99))} TL</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="grid md:grid-cols-2 gap-6 mb-8">
          <div className="bg-white rounded-lg shadow p-6">
            ...
            <address className="not-italic">
              ...
              <p className="text-gray-600">{t('phone')}: {shippingAddress.phone}</p>
            </address>
          </div>
          ...
          <div className="bg-white rounded-lg shadow p-6">
            ...
            <div>
              <p className="font-medium">{order.paymentMethod || t('creditCard') || 'Credit Card'}</p>
              <div className="mt-2 flex items-center">
              ...
              <p className="text-gray-600 mt-1">{t('total')}: {formatPrice(subtotal + (subtotal > 500 ? 0 : 29.99))} TL</p>
              {user?.isAdmin && (
                <div className="mt-4 pt-4 border-t">
                  <p className="text-sm text-gray-500">{t('adminNote') || 'Admin Note'}:</p>
                  <p className="text-sm">{t('orderId') || 'Order ID'}: {order.id}</p>
                  <p className="text-sm">{t('userId') || 'User ID'}: {order.userId}</p>
                </div>
              )}
            </div>
          </div>
        ...
        <div className="text-center">
          <p className="text-gray-600 mb-4">
            {t('trackOrderMessage') || 'You can track your order status on the'}{' '}
            <button onClick={handleViewMyOrdersClick} className="text-amber-600 hover:underline">
              {t('myOrders')}
            </button>{' '}
            {t('page') || 'page'}.
          </p>
